...
def query_coordinate(chainage: str) -> dict:
    """查询指定桩号的坐标"""
    db = get_graph_db()
...
def search_entities(keyword: str, entity_type: str = None, limit: int = 10) -> list:
    """搜索实体
    
    Args:
        keyword: 搜索关键词
        entity_type: 实体类型过滤 (Project/Drawing/Feature/Coordinate)
        limit: 返回结果数量
    ...
    try:
        results = db.execute_query(query, {"keyword": keyword, "limit": limit})
        entities = []
        for r in results:
            entity = dict(r.get("e", {}))
            entity["type"] = r.get("types", ["Unknown"])[0] if r.get("types") else "Unknown"
